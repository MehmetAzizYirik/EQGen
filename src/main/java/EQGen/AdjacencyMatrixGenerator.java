...
public class AdjacencyMatrixGenerator {
	...
	public static int[] setValues(IMolecularFormula formula) {
		...
		AdjacencyMatrixGenerator.valences=valences(acontainer);
		AdjacencyMatrixGenerator.isotopesCount=isotopes;
		return valences;
	}
	...
	public static List<int[]> rowsGen2(int index,int size) throws CloneNotSupportedException{
		List<int[]> rows= new ArrayList<int[]>();
		int[] zer= new int[size];
		rows.add(zer);
		int value=Math.min(4, size-index-1);
	...
	public static Set<int[][]> simpGen2(int index,int size,Set<int[][]> matrices,Set<int[][]> output) throws CloneNotSupportedException {
		if(index==size-1) {
			for(int[][] mat:matrices) {
				//output.add(mat);
				if(sum(mat)>=size-1 && zeroColumnCheckAll(mat,size)) { 
					if (outputCheck.add(mat)) output.add(mat);
				}
			}
		}else {
			if(index==0) {
				List<int[]> firstRows = rowsGen2(0,size); // to remove the duplicates I should add count down.
				...
				simpGen2((index+1),size,matrices,output);
			}else {
				for(int[][] mat:matrices) {
					//Set<int[][]> list= new HashSet<int[][]>();
					List<int[]> rows= rowsGen2(index,size);
					for(int[] arr:rows) {
						Set<int[][]> list= new HashSet<int[][]>();
						int[][] copy=copy(mat);
						setEntries(copy,index,arr);
						list.add(copy);
						simpGen2((index+1),size,list,output);
					}
				}
			}
		}
	...
	/**
	 * Valence check for an adjacency matrix
	 * @param mat int matrix
	...
	public static boolean zeroColumnCheckAll(int[][] mat,int size) {
		boolean check=true;
		for(int i=1;i<size;i++) {
	...
	public static int maxFill(int valence) {
		int count=0; 
		for(int i=0;i<size;i++) {
			count=count+Math.min(valence, (size-(i+1)));
		}
		return count;
	}
	...
	public static List<IAtomContainer> simpleAtomContainers(IMolecularFormula formula) throws CloneNotSupportedException, CDKException, IOException {
		long startTime = System.nanoTime(); //Recording the duration time.
		String formulaString =MolecularFormulaManipulator.getString(formula);
		AdjacencyMatrixGenerator.isotopes=formula.getIsotopeCount();
		...
		AdjacencyMatrixGenerator.size=ac.getAtomCount();
		AdjacencyMatrixGenerator.acontainer=ac;
		setValues(formula);
		...
		if(isotopes==1) {
			...
			for(int b= maxFill(valences[0]);b>size-1;b--) {
	...
	public static void main(String[] args) throws CloneNotSupportedException, CDKException, IOException  {	
		...
		AdjacencyMatrixGenerator.acontainer=acon;
		simpGen2(0, 8, mat, out);
		int count=0;
		int say=0;
		List<IAtomContainer> list= generateAtomContainers(out);
		long endTime = System.nanoTime()- startTime;
        double seconds = (double) endTime / 1000000000.0;
		DecimalFormat d = new DecimalFormat(".###");
		System.out.println(d.format(seconds));
		System.out.println(out.size());
		System.out.println(list.size());
		//System.out.println(out.size());
		
		for(int[][] m: out) {
			//System.out.println(say+" "+Arrays.deepToString(m));
			say++;
		}
		
		/**for(int i=0;i<list.size();i++) {
			depict(list.get(i),"C:\\Users\\mehme\\Desktop\\output\\"+count+".png");
			count++;
		}**/
	}
}
